...
public class BeanCounterLogicImpl implements BeanCounterLogic {
	...
	public int getInFlightBeanXPos(int yPos) {
		// TODO: Implement
		int temp = step-yPos;
		if(temp>=0 && temp < totBeans){
				//System.out.println("Y: "+ yPos + ", X: "+ beans[temp].getXPos());

				return beans[temp].getXPos();
		}
		//System.out.println("temp: " + step + ", yPos:" + yPos);
	...
	public void upperHalf() {
		...
		int newBeanIndex=0;
		Bean[] newBeans = new Bean[totBeans-tot];

		Boolean[] alive = new Boolean[totBeans];
		Arrays.fill(alive, Boolean.TRUE);
		
		for(int i=0;i<totBeans;i++){

			int index = 0;
		    for (int j = 0; j < totBeans; j++){
		        if (beans[i].getXPos() < beans[index].getXPos() && alive[i]) index = i;
		    }
		    if(tot>0){
		    	alive[index]=false;
		    	tot--;
		    	...
		    	alive[index]=false; 	
		    	newBeans[newBeanIndex] = beans[index];
		    	newBeanIndex++;
		    }
		...
		beans = newBeans;
		totBeans=beans.length;

		for (int j = 0; j < totBeans; j++){
			System.out.println(j+": "+beans[j].getXPos());
		}
	...
	public boolean advanceStep() {
		...
		for(int i=0; i<slotCount-1;i++){
			if(step-i>=0 && step-i<totBeans){
				//System.out.println(step-i);
				beans[step-i].choose();
			}
		}
				//System.out.println("step: " + step);
		int temp = step-slotCount+1;
		if(temp>=0 && temp< totBeans){
			int temp1 = beans[temp].getXPos();
		...
		// TODO: Implement
		return step<=totBeans+slotCount-1;
