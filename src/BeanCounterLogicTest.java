...
public class BeanCounterLogicTest {
	...
	public void testAdvanceStepBeanCount() {
		...
		while(move){
			move = logic.advanceStep();
			int totalBeans=0;
			totalBeans += logic.getRemainingBeanCount();
			for(int i=0;i<slotCount;i++){
				totalBeans += logic.getSlotBeanCount(i);
			}
			for(int i=0;i<slotCount;i++){
				if(logic.getInFlightBeanXPos(i) != NO_BEAN_IN_YPOS) totalBeans++;
			}
	...
	public void testAdvanceStepPostCondition() {
		...
		int flight=0;
		for(int i=0;i<slotCount;i++){
			if(logic.getInFlightBeanXPos(i) != NO_BEAN_IN_YPOS) flight++;
		}
		assertEquals(logic.getRemainingBeanCount(),0);
		assertEquals(flight,0);
		int totalBeans=0;
		for(int i=0;i<slotCount;i++){
			totalBeans += logic.getSlotBeanCount(i);
		}
